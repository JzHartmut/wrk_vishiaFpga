...
because it is a possible use case that different inner FPGA designs are used with the same pinning. 
...
==== Interface access to the pins

For that the link:Java2Vhdl_Approaches.html#IfcAccess[] should be used, of course. 
For specific pins a specific interface also with more as one bit can be used. For common ordinary pins the 
...
Hence this variables are [J]`public`. The input variables are also [J]`public` because they may be accessed by simple assignments too
depending on the top level design. 
...
`main/java/srcJava_FpgaExmpl...../modules/BlinkingLedCt.java`.
...
[#mdlPrc]
==== PROCESS inner classes in a module
...
* The sub class should be marked with the [J]`@Fpga.VHDL_PROCESS` annotation to detect as such from the Java2Vhdl translator.
* Variables in the sub class are generated in a type record with the class name:
