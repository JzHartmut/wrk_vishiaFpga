...
* The `step()` create a new instance using the `final` approach. The new instance is not used for other `step()`.
...
include::../../../main/java/srcJava_FpgaExmplBlinkingLed/org/vishia/fpga/exmplBlinkingLed/modules/BlinkingLedCt.java[tag=init]
----
...
//fpga/exmpl/modules/BlinkingLedCt.java
include::../../../main/java/srcJava_FpgaExmplBlinkingLed/org/vishia/fpga/exmplBlinkingLed/modules/BlinkingLedCt.java[tag=Q-Process-ce]
include::../../../main/java/srcJava_FpgaExmplBlinkingLed/org/vishia/fpga/exmplBlinkingLed/modules/BlinkingLedCt.java[tag=checktime]
      //.... logic working only if ce is set
    }
----
...
This is the content of a simple text file, able to view with any normal powerful text editor, for example link:https://notepad-plus-plus.org/[] or link:http://www.jedit.org/[]. 
...
The lines are time lines. Each column position in each line is the same time, more exact the start column of a longer information. 
Of course you should use a monospace font in the editor. 

But the time in the line may not be equidistant, depending of the output routines. 
...
Generally, adding a signal is only done if `clkDiv.q.ce` is true, because: This module changes only its state with this `ce` clock enable.
